<div wire:loading.class="opacity-50" href="#" class="flex p-2 mt-3 transition duration-300 ease-in-out bg-white rounded-lg hover:bg-gray-100 group">
    ...
    <div class="flex flex-col items-start justify-between w-2/3 px-2 basis-2/3">
        ...
        <div class="flex justify-between invisible w-full gap-1 opacity-0 group-hover:opacity-100 group-hover:visible group-hover:duration-300">
            @if (str_contains($collaboration->collaborationable_type, 'Episode'))
                <x-ui.button color="error" class="text-2xs py-0.5 px-1 grow text-white" href="{{ route('series-show', ['episode' => $collaboration->collaborationable->id, 'room' => $collaboration->room]) }}">  Watch Together
                </x-ui.button>
            @else
            <x-ui.button color="error" class="text-2xs py-0.5 px-1 grow text-white" href="{{ route('movie-show', ['movie' => $collaboration->collaborationable->id, 'room' => $collaboration->room]) }}"> Watch Together
            </x-ui.button>
            @endif
